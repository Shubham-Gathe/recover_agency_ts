...
const sectionStyles = [
    { title: 'Loan Agreement & Terms', color: '#00ACC1', icon: <AccountBalance />, fields: ['agreement_id', 'loan_amt', 'tenure', 'emi_amt', 'emi_od_amt'] },
    { title: 'Borrower & Contact Information', color: '#1E88E5', icon: <Person />, fields: ['customer_name', 'mobile', 'address', 'zipcode', 'phone1', 'phone2'] },
    { title: 'Collection & Payment History', color: '#D81B60', icon: <Payment />, fields: ['emi_coll', 'cbc_coll', 'total_coll', 'bcc_pending', 'penal_pending'] },
    { title: 'Officer & Agent Details', color: '#F57C00', icon: <SupervisorAccount />, fields: ['fos_name', 'fos_mobile_no', 'caller_name', 'caller_mo_number'] },
    { title: 'Operational & Routing Information', color: '#43A047', icon: <LocationOn />, fields: ['branch', 'pool', 'segment', 'cycle', 'pos'] },
        ...
        <Grid container spacing={2} style={{ padding: '20px' }}>
            {sectionStyles.map(({ title, color, icon, fields }, index) => (
                <Grid item xs={12} md={6} key={index}>
                    <Card style={{ borderRadius: '8px', border: `2px solid ${color}`, padding: '10px' }}>
                        <CardHeader
                            avatar={<Avatar style={{ backgroundColor: color }}>{icon}</Avatar>}
                            title={title}
                            titleTypographyProps={{ style: { color, fontWeight: 'bold' } }}
                        />
                        <CardContent>
                            {fields.map((field) => (
